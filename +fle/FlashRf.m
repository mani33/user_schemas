% fle.FlashRf This object computes one dimensional receptive field using the flashes in the
% flash lag experiment.
% MS 2012-02-04
%{
fle.FlashRf (computed) # my newest table
-> fle.BinnedSpikeSets
-> fle.SpikeBinParams
-> fle.FlashRfParams
-----
...
flashrf_ts = CURRENT_TIMESTAMP: timestamp   # importing time stamp
%}

classdef FlashRf < dj.Relvar & dj.AutoPopulate
    
    properties(Constant)
        table = dj.Table('fle.FlashRf')
    end
    
    properties
        popRel = fle.BinnedSpikeSets * fle.SpikeBinParams * fle.FlashRfParams;
    end
    ...
    methods
        ...
        function [peakInd SNR] = getCenter(self,varargin)
            ...
            for iUnit = 1:nUnits
                ...
                if args.fitCenter
                    ...
                    pVal = getSignificanceOfFit(fle.FlashRfFit(self & keys(iUnit)));
        ...
        function varargout = plot(self,varargin)
            ...
            % Show fit
            if args.showFit
                [b,fs] = getFitData(fle.FlashRfFit(self));
