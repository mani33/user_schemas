%{
fle.CombPsthFlash (computed) # Space time receptive field in the presense of motion
-> fle.BarGrayLevels
-> fle.CombPsthParams
-> fle.SpikeSets
-> fle.DxVals
...
classdef CombPsthFlash < dj.Relvar & dj.AutoPopulate
    ...
    properties(Constant)
        table = dj.Table('fle.CombPsthFlash')
        popRel = fle.BarGrayLevels * fle.DxVals * fle.CombPsthParams * fle.SpikeSets  % !!! update the populate relation
    end
    ...
    methods(Access=protected)
        ...
        function makeTuples(self, key)
            ...
            skey = fetch(fle.TrialGroup(key));
            [bar_width, nFlash] = fetch1(fle.StimConstants(skey),'bar_size_x','num_flash_locs_barmap');
            ...
            mon_type = fetch1(fle.StimConstants(skey),'monitor_type');
