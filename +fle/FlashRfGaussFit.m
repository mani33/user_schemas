% fle.FlashRfGaussFit - my newest table
% I will explain what my table does here

%{
fle.FlashRfGaussFit (computed) # my newest table
-> fle.FlashRf
...
classdef FlashRfGaussFit < dj.Relvar & dj.AutoPopulate
    
    properties(Constant)
        table = dj.Table('fle.FlashRfGaussFit')
    ...
    methods
        function self = FlashRfGaussFit(varargin)
        ...
        function makeTuples(self, key)
            ...
            key.fit_params = lsqcurvefit(@fle.FlashRfGaussFit.gauss,a,x,yi,[],[],opt);
            key.resid = (fle.FlashRfGaussFit.gauss(key.fit_params,x) - yi) / norm(yi);
