%{
flevbl.PsthFlash (computed) # my newest table
-> flevbl.PsthParams
-> flevbl.SpikeSets
...
psthflash_ts = CURRENT_TIMESTAMP: timestamp           # automatic timestamp. Do not edit
%}

classdef PsthFlash < dj.Relvar & dj.AutoPopulate
    
    properties(Constant)
        table = dj.Table('flevbl.PsthFlash')
        popRel = flevbl.PsthParams * flevbl.SpikeSets * flevbl.BarGrayLevels
    end
    
    methods
        function self = PsthFlash(varargin)
    ...
    methods(Access=protected)
        ...
        function makeTuples(self, key)
            ...
            [bar_width, nFlash] = fetch1(flevbl.StimConstants(key),'bar_size_x','num_flash_locs_barmap');
            ...
            for iFlash = 1:nFlash
                ...
                [stim_on, stim_off, trialSpikes{iFlash}] = fetchn(flevbl.SubTrials(skeys) * flevbl.SubTrialSpikes(key,skeys),'substim_on','substim_off','spike_times');
