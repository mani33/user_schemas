%{
fle.TrajControls (computed) # traj info for flash-initiated, -terminated and -reversed
-> fle.TrialGroup
...
classdef TrajControls < dj.Relvar & dj.AutoPopulate
    ...
    properties(Constant)
        table = dj.Table('fle.TrajControls')
        popRel = fle.TrialGroup & fle.StimConstants('combined = 0 and flash_init = 1 and flash_stop = 1 and flash_reverse = 1') & vstim.PixPerDeg
    end
    ...
    methods(Access=protected)
        ...
        function makeTuples(self, key)
            ...
            cond = fetch(fle.StimCond(key,'is_flash = 0 and is_moving = 1'),'*');
            for iCond = 1:length(cond) 
                cond_idx = cond(iCond).cond_idx;
                key.cond_idx = cond_idx;
                [trial_num, bar_centers, on, off, subtrial_num] = fetchn(fle.SubTrials(key)-fle.SubTrialsIgnore,'trial_num','bar_centers','substim_on','substim_off','subtrial_num');
                ppd = fetch1(vstim.PixPerDeg(key),'pix_per_deg');
                scon = fetch(fle.StimConstants(key),'*');
...
%                 assert(all(range(t,2) < 1),'time stamps range exceeds 1 ms')
                ...
                if nBad > 0
                    ...
                    for iBad = 1:nBad
                        ...
                        skey = fetch(fle.SubTrials(key,sprintf('subtrial_num = %u and trial_num = %u',sub_num,tnum)));
                        skey.reason = rea;
                        assert(length(key)==1,'more than one key to insert?')
                        insert(fle.SubTrialsIgnore,skey)
                    end
                end
                key.t = median(tg,2);
