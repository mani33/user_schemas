%{
fle.Traj (computed) # trajectory info - space and time
...
classdef Traj < dj.Relvar & dj.AutoPopulate
    ...
    properties(Constant)
        table = dj.Table('fle.Traj')
        popRel = fle.TrialGroup & fle.StimConstants('flash_init = 0 and flash_stop = 0') & vstim.PixPerDeg  % !!! update the populate relation
    end
    ...
    methods(Access=protected)
        ...
        function makeTuples(self, key)
            dxs = fetchn(fle.DxVals(key),'dx');
            nDx = length(dxs);
            comb = fetch1(fle.StimConstants(key),'combined');
            grays = fetchn(fle.BarGrayLevels(key),'bar_gray_level');
            nGray = length(grays);
                ...
                for iDx = 1:nDx
                    ...
                    for iGray = 1:nGray
                        ...
                        for iArr = 1:nArr
                            ...
                            [trial_num, bar_centers, on, off] = fetchn(fle.SubTrials(key,cs)-fle.SubTrialsIgnore,'trial_num','bar_centers','substim_on','substim_off');
