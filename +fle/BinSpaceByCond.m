%{
fle.BinSpaceByCond (computed) # my newest table
-> fle.BinTimesByCond
...
classdef BinSpaceByCond < dj.Relvar & dj.AutoPopulate
    ...
    properties(Constant)
        table = dj.Table('fle.BinSpaceByCond')
        popRel = fle.BinTimesByCond & (fle.TrajInfo | fle.RelFlashCenX)% !!! update the populate relation
    end
    ...
    methods(Access=protected)
        ...
        function makeTuples(self, key)
            ...
            ti = fetch1(fle.BinTimesByCond(key),'t');
            [flash,reverse] = fetchn(fle.StimCond(key),'is_flash','is_reverse');
            if flash
                loc = fetch1(fle.RelFlashCenX(key),'rel_to_mon_cen_deg');
