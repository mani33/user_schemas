% fle.FlashResp - my newest table
% I will explain what my table does here 

%{
fle.FlashRespStat (computed) # my newest table
-> fle.SpikeSets
-> fle.FlashRespStatParams
...
classdef FlashRespStat < dj.Relvar & dj.AutoPopulate
	...
	properties(Constant)
		table = dj.Table('fle.FlashRespStat')
	end
	properties
		popRel = fle.SpikeSets * fle.FlashRespStatParams  % !!! update the populate relation
	end
	...
	methods
        ...
        rfArr = getStimCenProxArrForCond(fle.TrialGroup(key),tc,'flash');
        ...
        for iFlash = 1:nFlashes
            ...
            spikes = fetchn(fle.SubTrialSpikes(key) & fle.SubTrials(cs),'spike_times');
