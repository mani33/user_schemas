%{
flevbl.BarRfCen (computed) # my newest table
-> flevbl.BarRfFit
...
classdef BarRfCen < dj.Relvar & dj.AutoPopulate
	...
	properties(Constant)
		table = dj.Table('flevbl.BarRfCen')
		popRel = flevbl.BarRfFit & flevbl.RelFlashCenX  % !!! update the populate relation
	end
	...
	methods(Access=protected)
		...
		%!!! compute missing fields for key here
            tup = key;
            fp = fetch1(flevbl.BarRfFit(key),'fit_params');
            if ~isempty(fp) 
                tup.cen_x_ind = fp(3);
                % Find corresponding visual space in degrees
                [loc_ind,loc_deg] = fetchn(flevbl.RelFlashCenX(key),'flash_location','rel_to_mon_cen_deg');
