...
%{
flevbl.SpikeSets (computed) # my newest table
-> flevbl.Phys
...
classdef SpikeSets < dj.Relvar & dj.AutoPopulate
    ...
    methods
        function [mfr_dir0 mfr_dir1] = getDirSelResp(self,dx,resp_win_start,resp_win_end)
            ...
            rfLocInd = getCenter(flevbl.FlashRf(key),'fitCenter',false);
            ...
            for direction = 0:1
                ...
                mCond = [cond.is_moving] & ~[cond.is_flash] & [cond.direction]==direction & ...
                    [cond.dx] == dx;
                mc = find(mCond,1);
                rfArr = getStimCenProxArrForCond(flevbl.TrialGroup(key),mc,'moving');
                mc = find(mCond & [cond.arrangement]==rfArr);
                
                zeroPoint = getTrajRelTimeAtFlashLoc(flevbl.TrialGroup(key),rfLocInd,dx,direction,...
                    misAlignTol,false);
                ...
                spikes = fetchn(flevbl.SubTrialSpikes(key) & flevbl.SubTrials(qs), 'spike_times');
        ...
        function [mfr_dir0 mfr_dir1] = getRfBoundaryBasedDirSelResp(self,dx,pre_rf_entry_time,post_rf_entry_time)
            ...
            rd = fetch(flevbl.FlashRespStat(key),'is_responsive','cond_idx');
            ...
            for direction = 0:1
                ...
                entryTimeLeft = getTrajRelTimeAtFlashLoc(flevbl.TrialGroup(key),rfBoundLeft,dx,direction,...
                    misAlignTol,false);
                
                entryTimeRight = getTrajRelTimeAtFlashLoc(flevbl.TrialGroup(key),rfBoundRight,dx,direction,...
                    misAlignTol,false);
