%{
flevbl.BarRfMovFit (computed) # my newest table
-> flevbl.BarRfMov
...
classdef BarRfMovFit < dj.Relvar & dj.AutoPopulate
    ...
    properties(Constant)
        table = dj.Table('flevbl.BarRfMovFit')
        fit_fun = '@(b,x) b(1) + b(2) * exp(-(x - b(3)).^2 / (2*(b(4)^2)))';
        popRel = flevbl.BarRfMov  % !!! update the populate relation
    end
    ...
    methods(Access = protected)
        function makeTuples(self, key)
            ...
            d = fetch(flevbl.BarRfMov(key),'base');
            sm = getSpatialMap(flevbl.BarRfMov(key));
            if any(isnan(sm))
                ...
                key.fit_params = lsqcurvefit(@flevbl.BarRfMovFit.gauss,a,x,yi,lb,ub,opt);
                key.resid = (flevbl.BarRfMovFit.gauss(key.fit_params,x) - yi) / norm(yi);
            end
            self.insert(key)
        end
    end
