%{
stim.RefreshPeriod (computed) # my newest table
...
classdef RefreshPeriod < dj.Relvar & dj.AutoPopulate
	...
	properties(Constant)
		table = dj.Table('stim.RefreshPeriod')
	end
	properties
	...
	methods
		...
		%!!! compute missing fields for key here
            ...
            elseif strcmp(monitorType,'LCD')
                ...
                T = median(ft);
                assert(T > 8 && T < 9,...
                    sprintf('Assuming 119 Hz monitor, the computed refreshPeriod (= %0.2d) is abnormal!',T));
            end
